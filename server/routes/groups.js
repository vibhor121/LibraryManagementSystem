  ...
  async (req, res) => {
    try {
      ...
      if (group.leader.toString() !== req.user._id.toString())
        return res
          .status(403)
          .json({ success: false, message: 'Only group leader can add members' });

      if (group.members.length >= group.maxMembers)
        return res
          .status(400)
          .json({ success: false, message: 'Group has reached maximum capacity' });
      ...
      if (existingGroup)
        return res
          .status(400)
          .json({ success: false, message: 'User is already in another group' });
  ...
  async (req, res) => {
    try {
      ...
      if (!canRemove)
        return res
          .status(403)
          .json({ success: false, message: 'You are not authorized to remove this member' });
      ...
      if (group.leader.toString() === memberId)
        return res.status(400).json({
          success: false,
          message: 'Leader cannot be removed. Transfer leadership first.',
        });
  ...
  async (req, res) => {
    try {
      ...
      if (group.leader.toString() !== req.user._id.toString())
        return res
          .status(403)
          .json({ success: false, message: 'Only leader can transfer leadership' });

      const isMember = group.members.some(m => m.toString() === newLeaderId);
      if (!isMember)
        return res
          .status(400)
          .json({ success: false, message: 'New leader must be a member' });
...
/**
 * @desc    Get all borrowed books for a group
 * @route   GET /api/groups/:groupId/borrowed-books
 * @access  Private
 */
router.get('/:groupId/borrowed-books', protect, async (req, res) => {
  try {
    ...
    const borrowedBooks = await BorrowRecord.find({
      borrower: { $in: group.members.map(m => m._id) },
      status: { $in: ['borrowed', 'overdue'] },
    })
